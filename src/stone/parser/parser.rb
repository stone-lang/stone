...
module Stone
  ...
  class Parser < Parslet::Parser
    ...
    rule(:literal) { boolean | integer | text }
    ...
    rule(:decimal_integer) { match["+-"].maybe >> match["0-9"].repeat(1) }
    rule(:hexadecimal_integer) { match["+-"].maybe >> str("0x") >> match["0-9a-fA-F"].repeat(1) }
    rule(:octal_integer) { match["+-"].maybe >> str("0o") >> match["0-7"].repeat(1) }
